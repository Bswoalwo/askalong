...
- **整篇理解**:打开一篇文章 → 侧边栏点「📝 总结全文」「🧱 提炼概念」「🎯 出题自测」。

## 工作原理

```
┌── 页面层 Content Script ─────────────────────────┐
│ 划词工具条 / 行内快答卡(Shadow DOM 样式隔离)      │
│ 正文提取器:平台适配器 → Readability兜底 → Markdown │
└─────────────┬───────────────────────────────┘
              │ chrome Port(请求 + 增量回推)
┌── 后台层 Service Worker ─────────────────────────┐
│ OpenAI 兼容请求 → fetch SSE → 逐行解析 → DELTA 回推│
│ 断开 Port = AbortController 中断生成              │
└─────────────┬───────────────────────────────┘
              │
┌── 展示层 ────────────────────────────────────────┐
│ 侧边栏(每标签页独立会话,storage.session 持久化)  │
│ 设置页(预设/Key/温度/上下文预算/测试连接)         │
└─────────────────────────────────────────────────┘
```
...
```
system  = 角色设定(优先依据文章作答、区分"文章说法"与"模型补充"、面向初学者)
        + <文章上下文 title/url/全文Markdown>
        + <用户划选片段>
history = 最近 12 轮对话
user    = 当前问题
```
...
├── e2e/                       # 端到端测试(mock LLM + puppeteer,见下)
...
测试会启动本地 mock LLM(SSE 流式),用真实 Chrome 加载扩展,自动完成:设置页保存 → 打开测试文章页 → 模拟划词 → 断言工具条出现 → 点「解释」→ 断言快答卡流式渲染 → 断言发给模型的请求**确实携带了全文、代码块、表格、列表、划选,且过滤了广告页脚** → 侧边栏对话与会话恢复。

> 注意:Chrome 137+ 正式版已禁用 `--load-extension`,测试需使用 Chrome for Testing。
> 首次运行前执行一次(约 190MB,已自动走国内镜像):
> ```bash
> cd e2e/.browsers
> curl -sS -O "https://cdn.npmmirror.com/binaries/chrome-for-testing/<版本号>/mac-arm64/chrome-mac-arm64.zip" && unzip -q chrome-mac-arm64.zip
> ```
> `run-test.mjs` 会自动优先使用 `.browsers/` 里的 Chrome for Testing,找不到才回退系统 Chrome(`CHROME_PATH` 可显式指定)。

## 常见问题

**Q: 为什么我的页面没有出现划词工具条?**
扩展安装后,**已打开的标签页需要刷新一次**才会注入脚本;另外该页面若是 `chrome://` 等浏览器内部页则不支持。

**Q: 提示"未提取到正文"?**
个别站点结构特殊,你可以直接划选想问的内容(划选内容始终会发给 AI),或到 issues 反馈站点名,我来加适配器。

**Q: 超长文章会超 token 吗?**
发送前有字符预算(默认 16000,设置页可调 2000–60000),超出自动按相关性截断并在提示词中告知模型「这是节选」。

**Q: 请求失败 401/429?**
401 = Key 无效;429 = 限流(免费模型高峰期常见),稍后重试或换模型。

**Q: 之前提示"连接中断"?**
v0.1.2 已修复。根因:思考类模型(如 glm-5.3-flash)出第一个字前要静默"思考"数十秒,而 Chrome MV3 的 Service Worker 空闲 30 秒会被强制休眠,数据通道随之断开。现在扩展会在请求进行期间自动保活,并把思考过程以「💭 已深度思考」折叠块流式展示。

## 路线图

- [ ] Firefox / Edge 官方支持
- [ ] 划线笔记沉淀为个人知识库(问过的概念自动积累,跨页面复用)
- [ ] 超长文档本地 RAG(向量检索后精准投喂)
- [ ] 多模型对比回答
- [ ] 移动端方案(分享菜单 / 剪贴板监听曲线版)

## 参与贡献

欢迎 Issue / PR,最有价值的贡献方向:

- **新增站点适配器**:在 `content/content.js` 的 `SITE_ADAPTERS` 里加一条该站正文选择器即可(3 行代码),这是社区最需要的;
- 思考类模型的思考块样式、长文截断策略优化;
- Firefox / Safari 移植。

开发调试:`chrome://extensions` 加载后改代码,点扩展卡片上的 ↻ 重载 + 刷新文章页即可生效;端到端测试见上文。
